...
export default function SectionBookingFlow({ onOpenCheckout }) {
  ...
  return (
    <section id="booking-flow" className="relative w-full py-16 md:py-24 bg-[#1A1008] border-t border-[#E8B89A]/40">
      <div className="max-w-[1440px] mx-auto px-4 sm:px-6 md:px-10 lg:px-12">
        ...
        {/* Main 2-Column Booking Widget */}
        <div className="w-full rounded-[24px] sm:rounded-[32px] overflow-hidden shadow-2xl border border-[#A64A2E]/30 grid grid-cols-1 lg:grid-cols-12 min-h-[640px]">
          
          {/* LEFT SIDEBAR: Gradient Background & 3 Steps with Radial Glow */}
          <div 
            className="lg:col-span-4 p-6 sm:p-8 md:p-10 flex flex-col justify-between"
            style={{
                ...
                return (
                  <div
                    ...
                    className={`group relative cursor-pointer rounded-[20px] p-4 sm:p-5 transition-all duration-300 ${
                      isActive 
                        ? 'bg-white/10 ring-2 ring-[#C97A3D] shadow-lg' 
                        : 'bg-black/20 hover:bg-white/5 opacity-85 hover:opacity-100'
                    }`}
                    style={{
                      backgroundImage: isActive 
                        ? 'radial-gradient(circle at center, rgba(201, 122, 61, 0.38) 0%, transparent 75%)' 
                        : 'none',
                    }}
                  >
                    {/* Hover radial glow overlay */}
                    <div className="absolute inset-0 rounded-[20px] opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity duration-300 bg-[radial-gradient(circle_at_center,rgba(201,122,61,0.35)_0%,transparent_70%)]" />

                    {/* Step Title in Serif */}
                    <div className="flex items-center justify-between mb-3 relative z-10">
                      <h3 className="font-playfair text-base sm:text-lg text-[#FFFFFF] font-medium tracking-[0.05em] uppercase">
                        {item.title}
                      </h3>
                      {isActive && (
                        <span className="w-2.5 h-2.5 rounded-full bg-[#C97A3D] shadow-[0_0_8px_#C97A3D]" />
                      )}
                    </div>
          ...
          {/* RIGHT CONTENT PANEL: #EFE6D8 Cream Background */}
          <div className="lg:col-span-8 bg-[#EFE6D8] p-6 sm:p-8 md:p-12 flex flex-col justify-between text-[#2B1810]">
            
            {/* STEP 1: Select Your Court */}
            {activeStep === 1 && (
              <div className="flex-1 flex flex-col">
                <div className="flex items-center justify-between mb-6 pb-3 border-b border-[#2B1810]/15">
                  <h3 className="font-playfair text-2xl sm:text-3xl font-medium uppercase text-[#2B1810] tracking-wide">
                    AVAILABLE COURTS
                  </h3>
                  ...
                  {courtsData.map((court) => {
                    ...
                    return (
                      <div
                        ...
                        className={`group relative cursor-pointer rounded-[18px] overflow-hidden transition-all duration-300 border ${
                          isSelected
                            ? 'ring-2 ring-[#6B1F22] border-[#6B1F22] shadow-xl scale-[1.01]'
                            : 'border-[#2B1810]/15 bg-[#F7F1E7] hover:border-[#C97A3D]'
                        }`}
                      >
                        <div className="relative h-44 sm:h-48 w-full overflow-hidden">
                          ...
                          {/* Top Badge */}
                          <div className="absolute top-3 left-3 bg-black/60 backdrop-blur-md px-3 py-1 rounded-full text-[11px] text-white font-serif tracking-wider">
                            {court.type}
                          </div>

                          <div className="absolute top-3 right-3 bg-[#6B1F22] text-white px-3 py-1 rounded-full text-xs font-semibold font-serif">
                            {court.rate}
                          </div>
                          ...
                          <div className="absolute bottom-3 left-4 right-4 flex items-end justify-between">
                            <div>
                              <h4 className="font-playfair text-2xl sm:text-3xl font-bold text-white tracking-wide drop-shadow-md">
                                {court.name}
                              </h4>
                              <p className="text-xs sm:text-sm text-white/80 font-serif line-clamp-1 mt-0.5">
                                {court.description}
                              </p>
                            </div>
                            <button
                ...
                <div className="mt-8 pt-4 border-t border-[#2B1810]/15 flex justify-end">
                  <button
                    type="button"
                    onClick={() => setActiveStep(2)}
                    className="px-8 py-3.5 rounded-full bg-[#6B1F22] hover:bg-[#A64A2E] text-white font-playfair font-medium text-base tracking-[0.03em] shadow-lg transition-all flex items-center gap-2"
                  >
            ...
            {activeStep === 2 && (
              <div className="flex-1 flex flex-col justify-between">
                <div>
                  <div className="flex items-center justify-between mb-4 pb-2 border-b border-[#2B1810]/15">
                    <h3 className="font-playfair text-2xl sm:text-3xl font-medium uppercase text-[#2B1810] tracking-wide">
                      SELECT YOUR DAY
                    </h3>
                    <span className="text-xs font-serif text-[#2B1810]/70 uppercase tracking-widest">
                  ...
                  <div className="relative border-2 border-dashed border-[#6B1F22]/40 rounded-[20px] p-6 sm:p-8 bg-[#F7F1E7]/70 shadow-sm">
                    {/* Month / Year Header */}
                    <div className="flex items-center justify-between mb-6 pb-3 border-b border-[#2B1810]/10">
                      <h4 className="font-playfair text-2xl sm:text-3xl font-bold text-[#2B1810] tracking-wide">
                          ...
                          <button
                            ...
                            className={`px-3 py-1 rounded-full text-xs font-serif transition-all ${
                              selectedMonth === m
                                ...
                                : 'bg-white/70 text-[#2B1810] hover:bg-[#C97A3D] hover:text-white'
                            }`}
                          >
                            {m}
                          </button>
                    ...
                    <div className="grid grid-cols-7 gap-2 mb-3 text-center">
                      {daysOfWeek.map((d) => (
                        <div key={d} className="font-sans text-[11px] sm:text-xs font-bold text-[#6B1F22] tracking-wider uppercase">
                          {d}
                        </div>
                      ))}
                    </div>
                      ...
                      {Array.from({ length: daysInMonth }).map((_, i) => {
                        ...
                        return (
                          <button
                            ...
                            className={`h-9 sm:h-11 rounded-full font-serif text-sm sm:text-base transition-all duration-200 flex items-center justify-center font-medium ${
                              isSelected
                                ? 'bg-[#6B1F22] text-white shadow-md scale-105 font-bold'
                                : 'bg-white/80 hover:bg-[#C97A3D] hover:text-white text-[#2B1810]'
                            }`}
                          >
                            {dayNum}
                          </button>
                ...
                <div className="mt-8 pt-4 border-t border-[#2B1810]/15 flex items-center justify-between">
                  <button
                    type="button"
                    onClick={() => setActiveStep(1)}
                    className="px-6 py-3 rounded-full bg-white text-[#2B1810] font-playfair text-sm hover:bg-[#2B1810] hover:text-white transition-all"
                  >
                    Back to Courts
                  </button>
                  <button
            ...
            {activeStep === 3 && (
              <div className="flex-1 flex flex-col justify-between">
                <div>
                  ...
                  <div className="border-2 border-dashed border-[#6B1F22]/40 rounded-[20px] p-6 sm:p-8 bg-[#F7F1E7]/70 shadow-sm">
                    <p className="font-serif text-xs text-[#2B1810]/70 uppercase tracking-widest mb-4">
                      ...
                      {timeSlots.map((slot) => {
                        ...
                        return (
                          <button
                            ...
                            className={`group flex items-center gap-3.5 px-5 py-3 rounded-full text-left transition-all duration-200 border ${
                              isSelected
                                ? 'bg-white border-[#6B1F22] ring-2 ring-[#6B1F22] shadow-md scale-[1.02]'
                                : 'bg-white/90 border-[#2B1810]/15 hover:border-[#C97A3D] text-[#2B1810]'
                            }`}
                          >
                            ...
                            <span className="font-serif text-sm sm:text-base font-semibold tracking-wide text-[#2B1810]">
                              {slot}
                            </span>
                          </button>
                        );
                ...
                <div className="mt-8 pt-4 border-t border-[#2B1810]/15 flex flex-col sm:flex-row items-center justify-between gap-4">
                  <button
                    type="button"
                    onClick={() => setActiveStep(2)}
                    className="px-6 py-3 rounded-full bg-white text-[#2B1810] font-playfair text-sm hover:bg-[#2B1810] hover:text-white transition-all w-full sm:w-auto"
                  >
                    Back to Date
                  </button>
                  ...
                  <button
                    ...
                    className={`px-10 py-4 rounded-full font-playfair font-medium text-lg tracking-[0.03em] shadow-2xl transition-all duration-300 w-full sm:w-auto text-center ${
                      isFormComplete
            ...
            {isBookedSuccess && (
              <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-md flex items-center justify-center p-4">
                <div className="bg-[#EFE6D8] max-w-md w-full rounded-[24px] p-8 border-2 border-[#6B1F22] shadow-2xl text-center">
                  ...
                  <p className="font-serif text-sm text-[#2B1810]/80 mb-6">
                  ...
                  <button
                    ...
                    className="w-full py-3.5 rounded-full bg-[#6B1F22] hover:bg-[#A64A2E] text-white font-playfair text-base tracking-wider transition-all"
                  >
                    Done
                  </button>
                </div>
